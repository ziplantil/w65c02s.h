...
INLINE void interrupt_check(struct w65c02s_cpu *cpu) {
    cpu->do_nmi |= cpu->nmi;
    cpu->do_irq = cpu->irq && !GET_P(P_I);
}

INLINE void interrupt_reset(struct w65c02s_cpu *cpu) {
    if (cpu->reset) {
        cpu->reset = 0;
    } else if (cpu->in_nmi) {
        cpu->in_nmi = 0;
...
/* 0 = no penalty, 1 = decimal mode penalty */
INLINE int oper_imm(struct w65c02s_cpu *cpu, uint8_t v) {
    switch (cpu->oper) {
        case OPER_NOP: break;
...
/* 0 = no penalty, 1 = decimal mode penalty */
INLINE int oper_addr(struct w65c02s_cpu *cpu, uint16_t a) {
    switch (cpu->oper) {
        case OPER_NOP: break;
...
INLINE int mode_implied(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            interrupt_check(cpu);
            switch (cpu->oper) {
                ...
                case OPER_TAX: cpu->x = update_flags(cpu, cpu->a); break;
                case OPER_TAY: cpu->y = update_flags(cpu, cpu->a); break;
                case OPER_TSX: cpu->x = update_flags(cpu, cpu->s); break;
                case OPER_TXA: cpu->a = update_flags(cpu, cpu->x); break;
                case OPER_TXS: cpu->s = cpu->x;                    break;
                case OPER_TYA: cpu->a = update_flags(cpu, cpu->y); break;
                case OPER_NOP:                                     break;
            }
            READ(cpu->pc);
    END_INSTRUCTION
...
INLINE int mode_implied_x(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            interrupt_check(cpu);
...
INLINE int mode_implied_y(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            interrupt_check(cpu);
...
#define ADC_D_SPURIOUS(ea)                                                     \
    if (!cpu->tr[5]) SKIP_REST; /* no penalty cycle => skip last read */       \
    interrupt_check(cpu);                                                      \
    cpu->p = cpu->p_adj;                                                       \
    READ(ea);

INLINE int mode_immediate(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            interrupt_check(cpu);
            cpu->tr[5] = oper_imm(cpu, READ(cpu->pc++)); /* penalty cycle? */
        CYCLE(2):
            ADC_D_SPURIOUS(cpu->pc - 1);
    END_INSTRUCTION
}

INLINE int mode_zeropage(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
            interrupt_check(cpu);
            cpu->tr[5] = oper_addr(cpu, cpu->tr[0]); /* penalty cycle? */
        CYCLE(3):
            ADC_D_SPURIOUS(cpu->tr[0]);
    END_INSTRUCTION
}

INLINE int mode_zeropage_x(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc);
        CYCLE(2):
...
INLINE int mode_zeropage_y(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc);
        CYCLE(2):
...
INLINE int mode_absolute(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
        ...
        CYCLE(3):
            interrupt_check(cpu);
            cpu->tr[5] = oper_addr(cpu, GET_T16(0)); /* penalty cycle? */
        CYCLE(4):
            ADC_D_SPURIOUS(GET_T16(0));
    END_INSTRUCTION
}

INLINE int mode_absolute_x(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
        ...
        CYCLE(3):
            ...
            if (!cpu->tr[4] && !oper_is_store(cpu)) SKIP_TO_NEXT(4);
            READ(cpu->pc - 1);
            cpu->tr[1] += cpu->tr[4];
        CYCLE(4):
            interrupt_check(cpu);
...
INLINE int mode_absolute_y(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
...
INLINE int mode_zeropage_indirect(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
...
INLINE int mode_zeropage_indirect_x(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[2] = READ(cpu->pc);
        CYCLE(2):
...
INLINE int mode_zeropage_indirect_y(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[2] = READ(cpu->pc++);
        CYCLE(2):
        ...
        CYCLE(4):
            /* if we did not cross the page boundary, skip this cycle */
            if (!cpu->tr[4] && !oper_is_store(cpu)) SKIP_TO_NEXT(5);
            READ(cpu->pc - 1);
            cpu->tr[1] += cpu->tr[4];
        CYCLE(5):
            interrupt_check(cpu);
...
INLINE int mode_jump_absolute(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
            interrupt_check(cpu);
...
INLINE int mode_jump_indirect(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
...
INLINE int mode_jump_indirect_x(struct w65c02s_cpu *cpu) {
    unsigned overflow;
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
...
INLINE int mode_zeropage_bit(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
...
INLINE void mode_relative_i(struct w65c02s_cpu *cpu) {
    /* offset in tr[0] */
    /* old PC in tr[0], tr[1] */
    /* new PC in tr[2], tr[3] */
...
INLINE int mode_relative(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
            mode_relative_i(cpu);
...
INLINE int mode_relative_bit(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
        ...
        CYCLE(3):
            ...
            mode_relative_i(cpu);
        CYCLE(4):
            cpu->tr[5] = w65c02si_oper_bitbranch(cpu->oper, cpu->tr[4]);
            READ(cpu->pc - 1);
            interrupt_check(cpu);
        CYCLE(5):
            /* skip the rest of the instruction if branch is not taken */
            if (!cpu->tr[5]) SKIP_REST;
            READ(GET_T16(0));
            cpu->pc = GET_T16(2);
            interrupt_check(cpu);
        CYCLE(6):
...
INLINE int mode_rmw_zeropage(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
...
INLINE int mode_rmw_zeropage_x(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc);
        CYCLE(2):
...
INLINE int mode_rmw_absolute(struct w65c02s_cpu *cpu) {
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
        CYCLE(2):
...
INLINE int mode_rmw_absolute_x(struct w65c02s_cpu *cpu) {
    unsigned overflow;
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
...
INLINE int mode_stack_push(struct w65c02s_cpu *cpu) {
    uint8_t tmp;
    BEGIN_INSTRUCTION
        CYCLE(1):
            READ(cpu->pc);
...
INLINE int mode_stack_pull(struct w65c02s_cpu *cpu) {
    uint8_t tmp;
    BEGIN_INSTRUCTION
        CYCLE(1):
            READ(cpu->pc);
...
INLINE int mode_subroutine(struct w65c02s_cpu *cpu) {
    /* op = JSR */
    BEGIN_INSTRUCTION
        CYCLE(1):
            cpu->tr[0] = READ(cpu->pc++);
...
INLINE int mode_return_sub(struct w65c02s_cpu *cpu) {
    /* op = RTS */
    BEGIN_INSTRUCTION
        CYCLE(1):
            READ(cpu->pc);
...
INLINE int mode_stack_rti(struct w65c02s_cpu *cpu) {
    /* op = RTI */
    BEGIN_INSTRUCTION
        CYCLE(1):
            READ(cpu->pc);
...
static int mode_stack_brk(struct w65c02s_cpu *cpu) {
    ...
    uint16_t vec = VEC_IRQ;
    int rst = cpu->reset, brk = 0;
    uint8_t tmp;
    ...
    BEGIN_INSTRUCTION
        ...
        CYCLE(5):
            interrupt_reset(cpu);
            SET_LO(cpu->pc, READ(vec));
        CYCLE(6):
            SET_HI(cpu->pc, READ(vec + 1));
    END_INSTRUCTION
}

INLINE int mode_int_wait(struct w65c02s_cpu *cpu) {
    /* op = WAI */
    BEGIN_INSTRUCTION
        CYCLE(1):
            READ(cpu->pc);
...
INLINE int mode_int_stop(struct w65c02s_cpu *cpu) {
    /* op = STP */
    BEGIN_INSTRUCTION
        CYCLE(1):
            READ(cpu->pc);
#if W65C02SCE_HOOK_STP
            if (cpu->hook_stp && *(cpu->hook_stp)()) SKIP_REST;
#endif
        CYCLE(2):
            READ(cpu->pc);
        CYCLE(3):
            cpu->stp = 1;
            return 1; /* exit instantly since we entered STP */
    END_INSTRUCTION
}
...
INTERNAL int w65c02si_run_mode(struct w65c02s_cpu *cpu) {
    ...
    case MODE_IMPLIED_1C:             /* finishes immediately */
    ...
    case MODE_INT_WAIT:               return mode_int_wait(cpu);
    case MODE_INT_STOP:               return mode_int_stop(cpu);
