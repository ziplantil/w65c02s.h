...
#if __GNUC__ >= 5
#define WARP_FIRST              if (__builtin_constant_p(cont) && !cont) \
                                    goto cycle_1;
#else
#define WARP_FIRST              if (LIKELY(!cont)) goto cycle_1;
#endif

#define SKIP_REST               return 0
#define SKIP_TO_NEXT(n)         do { ++cpu->cycl; goto cycle_##n; } while (0)
#define BEGIN_INSTRUCTION       WARP_FIRST                                     \
                                switch (cpu->cycl) {
#define CYCLE_END               CYCLE_TOTAL_INC                                \
                                if (UNLIKELY(!--cpu->left_cycles)) return 1;
#define CYCLE_END_LAST          CYCLE_TOTAL_INC                                \
                                if (UNLIKELY(!--cpu->left_cycles)) {           \
                                    cpu->cycl = 0;                             \
                                    return 1;                                  \
                                }
